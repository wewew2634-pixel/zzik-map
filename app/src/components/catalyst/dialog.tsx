...
export function Dialog({
  ...
  onClose,
  ...props
}: { size?: keyof typeof sizes; className?: string; children: React.ReactNode; open?: boolean; onClose?: (value: boolean) => void } & Omit<
  Headless.DialogProps,
  'as' | 'className' | 'open' | 'onClose'
>) {
  const previousActiveElement = useRef<HTMLElement | null>(null)

  // Lock scroll and manage focus when dialog opens
  useEffect(() => {
    if (open) {
      ...
      // Set initial focus to dialog (for screen readers)
      return () => {
        // Unlock body scroll
  ...
  return (
    <Headless.Dialog {...props} aria-modal="true" open={open} onClose={onClose ?? (() => {})}>
